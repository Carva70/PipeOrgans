...
public class RohrfloteSoundInstance extends AbstractTickableSoundInstance {
    ...
    public RohrfloteSoundInstance(Generic.WhistleSize size, BlockPos worldPosition) {
        super((size == Generic.WhistleSize.TINY ? ROHRFLOTE_SUPERHIGH :
                size == Generic.WhistleSize.SMALL ? ROHRFLOTE_HIGH :
                size == Generic.WhistleSize.MEDIUM ? ROHRFLOTE_MEDIUM :
                size == Generic.WhistleSize.LARGE ? ROHRFLOTE_LOW : ROHRFLOTE_DEEP).get(),
                SoundSource.RECORDS,
